...
std::vector<Matrix> KinematicModel::stateSpace(double delta_ref, double yaw_ref) {
    ...
    */
    ...
    A << 1.0, 0.0, -v_ * sin(yaw_ref) * dt_,
         0.0, 1.0,  v_ * cos(yaw_ref) * dt_,
         0.0, 0.0, 1.0;
    B << cos(yaw_ref) * dt_, 0.0,
         sin(yaw_ref) * dt_, 0.0,
         tan(delta_ref) / l_ * dt_,  v_ * dt_ / (l_ * cos(delta_ref) * cos(delta_ref));
    return {A, B};     
}
