...
ReferencePath::ReferencePath() {
  ...
  for (int i = 0; i < refer_path.size(); i++) {
    ...
    refer_path[i][3] =
        (ddy * dx - ddx * dy) / pow((dx * dx + dy * dy), 3 / 2);  //  kappa
  }
}

std::vector<double> ReferencePath::CalcRefTrajectory(
    const std::vector<double> vehicle_state) {
...
RefTraj ReferencePath::calcRefTrajectory(std::vector<double> vehcile_state,
                                         Parameters param, double dl) {
  std::vector<double> result = CalcTrackError(vehcile_state);
  double err = result[0], kappa = result[1], yaw = result[2],
         min_index = result[3];
